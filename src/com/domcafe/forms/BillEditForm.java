 ...
 * @author admin
 */
public class BillEditForm extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        txtbillno.setFont(new java.awt.Font("Verdana", 0, 12)); // NOI18N
        txtbillno.setForeground(new java.awt.Color(153, 0, 51));
                    ...
                    .addGroup(jPanel5Layout.createSequentialGroup()
                        ...
                        .addGroup(jPanel5Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                            .addGroup(jPanel5Layout.createSequentialGroup()
                                ...
                                .addGap(18, 18, 18)
                                .addGroup(jPanel5Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                                    .addComponent(txtbillno, javax.swing.GroupLayout.PREFERRED_SIZE, 111, javax.swing.GroupLayout.PREFERRED_SIZE)
   ...
   public void setData(String billno){
        try{
        ResultSet rs=db.getData("select * from Invoice where BillNo=?",billno);
        if(rs.next()){
            txtbillno.setText(rs.getString(1));
            txtdate2.setDate(LocalDate.parse(rs.getString(2)));
            cmbtbl.setSelectedItem(rs.getObject(3));
            
            txtname.setText(rs.getString(4));
            txtaddress.setText(rs.getString(5));
            txtmobileno.setText(rs.getString(6));
            txtdiscp2.setText(rs.getString(7));
            txtdisc2.setText(rs.getString(8));
            cmbcash.setSelectedItem(rs.getObject(9));
            txttotalamt2.setText(rs.getString(10));
            txtnetamt2.setText(rs.getString(11));
           
            
            db.fillTable(tab3, "select ItemNo,ItemName,Qty,gst,Price,Total From invitem where BillNo=?",billno);
        }
        }catch(Exception ex){
            ex.printStackTrace();
        }
    ...
    private void btnSave2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnSave2ActionPerformed
        ...
        //Master Record Entry
        db.execute("Update Invoice set InvoiceDate=?,TableNo=?,Name=?,Address=?,Mobile=?,DisP=?,DisAmt=?,Total=?,NetAmt=? where BillNo=?",qdate,tabl,customer,address,mobile,discp,discamt,tamt,namt,billno);
        ...
        for(int i=0;i<tab3.getRowCount();i++){
            ...
            db.execute("Update invitem Set ItemNo=?,ItemName=?,Qty=?,gst=?,Price=?,Total=? where BillNo=?",itemno,itemname,qty,gst,price,total,billno);
        }
        //Update ID
       
//        db.fillTable(parent.table1, "select * Invoice");
        Utility.showMsg("Invoice Updated Successfully..", "Success");
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(BillEditForm.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new BillEditForm().setVisible(true);
            }
        });
    }
